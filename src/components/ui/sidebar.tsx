...
export default function Sidebar() {
  ...
  return (
    <aside className={`relative flex flex-col h-screen flex-shrink-0 transition-all duration-300 ${collapsed ? 'w-[64px]' : 'w-52'} glass-sidebar border-r border-border/50`}>
      {/* Logo */}
      <div className={`flex items-center gap-2.5 ${collapsed ? 'justify-center px-0 py-5' : 'px-5 py-5'} border-b border-border/40`}>
        ...
        {!collapsed && (
          <span className="font-bold text-base text-foreground">Finanças</span>
        )}
      </div>
      ...
      {/* Footer */}
      <div className="px-2 py-3 border-t border-border/40 space-y-1.5">
        <ThemeToggle isCollapsed={collapsed} className={collapsed ? 'w-full justify-center' : 'w-full'} />
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs text-muted-foreground hover:bg-secondary/60 transition"
        >
